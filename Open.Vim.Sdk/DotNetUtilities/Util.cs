...
namespace Vim.DotNetUtilities
{
    ...
    public static unsafe class Util
    {
        ...
        public static string TimeStampedFileName(this string path)
            => Path.Combine(Path.GetDirectoryName(path), $"{Path.GetFileNameWithoutExtension(path)}-{GetTimeStamp()}{Path.GetExtension(path)}");
        ...
        public static IDictionary<string, object> FieldsToDictionary(this object self)
            => self.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance).ToDictionary(fi => fi.Name, fi => fi.GetValue(self));
        ...
        public static IDictionary<string, object> PropertiesToDictionary(this object self)
            => self.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance).ToDictionary(fi => fi.Name, fi => fi.GetValue(self));
        ...
        /// Given a dictionary looks up the key, or uses the function to add to the dictionary, and returns that result.
        /// </summary>
        public static V GetOrCompute<K, V, TContext>(this IDictionary<K, V> self, TContext context, K key, Func<TContext, K, V> func)
        {
            if (self.ContainsKey(key))
                return self[key];
        ...
        /// </summary>
        public static IEnumerable<FileInfo> GetFilesInDirectoryWhere(string dirPath, Func<FileInfo, bool> predicateFn, bool recurse = true)
            => !Directory.Exists(dirPath)
                ? Enumerable.Empty<FileInfo>()
                : Directory.GetFiles(dirPath, "*", recurse ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly)
        ...
        public static CustomTextWriter Tee(TextWriter original, TextWriter other)
            => new CustomTextWriter((c) => { original.Write(c); other.Write(c); });
        ...
        public static Disposer RedirectOut(TextWriter tw)
        {
            ...
            Console.SetOut(tw);
            return Disposer(() => Console.SetOut(original));
        }
        ...
        /// </summary>
        public static T TryGetValue<T>(Func<T> getValue, T defaultValue, Action<Exception> onExceptionAction)
        {
            try
            {
                return getValue();
        ...
        /// </summary>
        public static DirectoryInfo FindParentDirectory(DirectoryInfo currentDirectory, Func<DirectoryInfo, bool> predicate, int maxIterations = 15)
        {
        ...
        public static void PopIfNotEmpty<T>(this Stack<T> self)
        {
            if (self.Count > 0)
                self.Pop();
        }
        ...
        public static Dictionary<U, T> MapElementsToKeys<T, U>(this IEnumerable<IGrouping<T, U>> self)
            => self.SelectMany(grp => grp.Select(element => (grp.Key, element))).ToDictionary(pair => pair.element, pair => pair.Key);
        ...
        public static string ToSqlServerAddConstraints(DataTable table)
        {
