...
namespace Vim.DotNetUtilities
{
    ...
    public static class IJSonSerializerExtensions
    {
        ...
        /// </summary>
        public static T LoadJson<T>(this IJsonSerializer serializer, string folder, string file) where T : class
        {
            var inputFile = Path.Combine(folder, file);
            return serializer.LoadJsonFromFile<T>(inputFile);
        }

        /// <summary>
        /// Loads the object from the given folder and file. The first parameter acts as a type inference helper for code ergonomics.
        /// </summary>
        public static T LoadJson<T>(this IJsonSerializer serializer, T x, string folder, string file) where T : class
            => serializer.LoadJson<T>(folder, file);

        /// <summary>
        /// Loads the object from the given stream.
        /// </summary>
        public static T LoadJson<T>(this IJsonSerializer serializer, Stream stream) where T : class
        ...
        /// </summary>
        public static T LoadJson<T>(this IJsonSerializer serializer, T x, Stream stream) where T : class
            => serializer.LoadJson<T>(stream);

        /// <summary>
        /// Loads the object from the given zip archive.
        /// </summary>
        public static T LoadJson<T>(this IJsonSerializer serializer, ZipArchive archive, string entryName) where T : class
        {
            using (var stream = archive.GetEntry(entryName)?.Open())
            {
                if (stream == null) return default;
                return serializer.LoadJson<T>(stream);
            }
        }
        ...
        /// </summary>
        public static T LoadJson<T>(this IJsonSerializer serializer, T x, ZipArchive archive, string entryName) where T : class
            => serializer.LoadJson<T>(archive, entryName);
    }
}
