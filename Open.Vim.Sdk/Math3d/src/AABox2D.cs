...
namespace Vim.Math3d
{
    public partial struct AABox2D
    {
        ...
        public static readonly AABox2D Empty
            = new AABox2D(Vector2.MaxValue, Vector2.MinValue);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public AABox2D Merge(AABox2D box)
            => new AABox2D(Min.Min(box.Min), Max.Max(box.Max));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public AABox2D Merge(Vector2 point)
            => new AABox2D(Min.Min(point), Max.Max(point));
